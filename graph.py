...
def plot_summary_figure(
    ...
    df_coeff_input, df_coeff_output,
    ...
    ax_summary_in.text(0.05, 0.96, "Power Summary", transform=ax_summary_in.transAxes, 
                         fontsize=12, va='top', weight='bold')
    ax_summary_in.text(0.05, 0.9, summary_text, transform=ax_summary_in.transAxes, 
                         fontsize=10, va='top', fontfamily='monospace')

    # Input coefficients table
    ax_summary_in.text(0.05, 0.55, r"Input Coefficients ($|A|^2$ %)", transform=ax_summary_in.transAxes, 
                         fontsize=12, va='top', weight='bold')
    ...
    try:
        ...
        for row in table_data_in:
            ...
            formatted_row = [f"{row[0]:.0f}", f"{row[1]:.0f}"] + [f"{x:.1f}" for x in row[2:]]
            formatted_data_in.append(formatted_row)

        column_labels_in = ['l', 'm'] + df_plot_in.columns.tolist()
        
        table_in = ax_summary_in.table(cellText=formatted_data_in, colLabels=column_labels_in, 
        ...
        ax_summary_in.text(0.0, 0.3, f"Error creating input table: {e}", 
                             transform=ax_summary_in.transAxes, color='red')
    ...
    ax_summary_out.text(0.05, 0.85, r"Output Coefficients  ($|A|^2$ %)", transform=ax_summary_out.transAxes, 
                          fontsize=12, va='top', weight='bold')
    ...
    try:
        ...
        table_out = ax_summary_out.table(cellText=formatted_data_out, colLabels=column_labels_out, 
                                         loc='center', cellLoc='center',
                                         bbox=[0.05, 0.3, 0.90, 0.45]) # <-- Reduced height from 0.85 to 0.45
        ...
        ax_summary_out.text(0.0, 0.5, f"Error creating output table: {e}", 
                              transform=ax_summary_out.transAxes, color='red')
    ...
    cbar = fig.colorbar(im_out, ax=[ax_guided, ax_propagated], shrink=0.8, aspect=30, pad=0.02)
